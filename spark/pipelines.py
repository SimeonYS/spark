...
class SparkPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `spark`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM spark WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO spark (date, title, link, content)"
